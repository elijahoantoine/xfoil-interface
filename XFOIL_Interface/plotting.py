...
def plot_liftvsAoA(aoa_values, lift_coeffs, airfoil_name, desired_aoas, aoa_step, results, reynolds=None, xlim=None, ylim=None):
    ...
    ax.plot(aoa_values, lift_coeffs, 'g-', linewidth=1.5, label=r'$C_L$ curve')
    ...
    if aoa_step > 0.2:
        ...
        ax.plot(desired_aoas_clean, desired_cls, 'go', markersize=5, label=r'Selected $\alpha$ points')

    ax.set_xlabel(r'$\alpha$ (degrees)')
    ax.set_ylabel(r'Lift Coefficient, $C_L$')
    re_str = f" - Re = {reynolds:.2e}" if reynolds is not None else ""
    ax.set_title(f'$C_L$ vs $\\alpha$: {label}{re_str}')
    ax.legend()
    if xlim:
        ax.set_xlim(xlim)
    if ylim:
...
def plot_dragpolar(drag_coeffs, lift_coeffs, airfoil_name, reynolds=None, xlim=None, ylim=None):
...
def plot_Cp_distribution(cp_x, cp_values, airfoil_coords, airfoil_name, aoa, cl, cd, cm, cdp, reynolds=None, experimental_data=None):
    ...
    re_str = f" - Re = {reynolds:.2e}" if reynolds is not None else ""
    ax_cp.set_title(f'$C_p$ Distribution: {label} — $\\alpha$ = {aoa}°{re_str}')
    ...
    if airfoil_coords and len(airfoil_coords[0]) > 0:
        ...
        ax_af.set_xlabel(r'$x/c$')
        ax_af.set_ylabel(r'$y/c$')
        ax_af.grid(True)

    plt.tight_layout()
    return fig

def plot_liftvsAoA_multi(all_results, airfoil_name, experimental_data=None, xlim=None, ylim=None):
    ...
    ax.set_xlabel(r'$\alpha$ (degrees)')
    ax.set_ylabel(r'Lift Coefficient, $C_L$')
    ax.set_title(f'$C_L$ vs $\\alpha$: {label} — Multi-Re Comparison')
    if xlim:
        ax.set_xlim(xlim)
    if ylim:
        ax.set_ylim(ylim)
...
def plot_dragpolar_multi(all_results, airfoil_name, experimental_data=None, xlim=None, ylim=None):
...
def plot_Cp_multi(cp_data, airfoil_coords, airfoil_name, reynolds, experimental_data=None):
    ...
    for i, (aoa, (cp_x, cp_values)) in enumerate(sorted(cp_data.items())):
        ax_cp.plot(cp_x, cp_values, linewidth=1.5,
                   label=f'$\\alpha$ = {aoa}°', color=colors[i % len(colors)])
